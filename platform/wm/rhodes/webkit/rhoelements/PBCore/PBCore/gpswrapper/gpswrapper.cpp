...
CGpsWrapper::CGpsWrapper(IGpsWrapperObserver& observer):
	mObserver(observer)
{
...
BOOL CGpsWrapper::Init()
{
	...
	*/
	...
	if (!m_pGpsIdDll)
	{
		Log(PB_LOG_ERROR, L"gpsapi.dll not available!", 
			_T(__FUNCTION__), __LINE__, L"CGpsWrapper");		
		return FALSE;
	}
	//Check whether GPS hardware is present
	if (!m_hDeviceStateChangedEvent)
		m_hDeviceStateChangedEvent  = CreateEvent(NULL,FALSE,FALSE,NULL);
	WaitForSingleObject(m_hDeviceStateChangedEvent, GPSINFO_WAIT_TIMEOUT);
	GPS_DEVICE sState;
	memset(&sState, 0, sizeof(GPS_DEVICE));
	sState.dwVersion = GPS_VERSION_1;
	sState.dwSize = sizeof(GPS_DEVICE);
	GPSGetDeviceStateType pfGPSGetDeviceState = (GPSGetDeviceStateType)GetProcAddress(m_pGpsIdDll, L"GPSGetDeviceState");
	DWORD err = ERROR_SUCCESS;
	if((err = pfGPSGetDeviceState(&sState)) == ERROR_SUCCESS)
	{
		if (sState.dwDeviceState == SERVICE_STATE_OFF) //No GPS hw
		{
			//unload gpsapi.dll
			FreeLibrary(m_pGpsIdDll);
			m_pGpsIdDll = NULL;
...
DWORD CGpsWrapper::GpsListener(LPVOID lParam)
{
	...
	if (!pfGPSGetPosition)
	{
		Log(PB_LOG_ERROR, L"Initialisation error", 
			_T(__FUNCTION__), __LINE__, L"CGpsWrapper");	
		return S_FALSE;
	}
	...
	while (pGpsWrapper->m_bThreadActive)
	{
		...
		switch (dwWaitRes)
		{
			...
			case WAIT_TIMEOUT:
				...
				//Increase GpsWrapper timeout counter
				if (++pGpsWrapper->m_uTimeoutCount == 3)
				{
					pGpsWrapper->m_uTimeoutCount = 0; //reset the counter
					pGpsWrapper->m_bThreadActive = FALSE;
					DEBUGMSG(TRUE, (TEXT("%s, %d -- Timeout occurred too many times. Gps listener will now terminate\n"), _T(__FUNCTION__), __LINE__));
					return S_FALSE;
				}
				break;
