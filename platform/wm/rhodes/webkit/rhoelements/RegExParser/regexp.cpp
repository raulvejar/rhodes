...
////////////////////////////////////////////////////////////////////////////////
//
//  Modifications for PocketBrowser 3.0, 2008
...
//  exactly (operand to the EXACTLY operator), e.g. PocketBrowser30.  
#define	META		"^$.[()|?+*\\"
...
//  ToDo - point these to the PocketBrowser SpbLog file
...
CRegExp::~CRegExp()
{
	//  Free memory associated with compiled regular expression, modified for PocketBrowser,
	//  previous version was missing []
	delete[] m_compiledExpression;		
	//  Free memory associated with (ToDo - what is this?), modified for 
	//  PocketBrowser, previous version was missing []
...
CRegExp* CRegExp::CompileRegularExpression(const char *regularExpression)
{
	...
	// Allocate space. 
	//  PocketBrowser modification - improved memory management
	//  Delete any previous compiled expression
	delete[] m_compiledExpression;
	m_compiledExpression = NULL;	
	//  End PocketBrowser modifications
	...
	*/
	...
	{	
		...
		//  PocketBrowser: The following comment which came shipped with the
		...
		while ((c = *m_regularExpression++) != '\0' && c != ']') {
			if (c != '-')
				WriteCharacterToExpression(tolower(c));
							//  PocketBrowser - Made Case Insensitive
...
//	int offset;
	...
	//  Modifications made by PocketBrowser:
	...
	*(scan+2)=*(cp+1);*/
	...
	//  PocketBrowser - Andreas Phol's change works on mobile devices, the original code
...
//#else
	...
	//  new behaviour for PocketBrowser
	*sp=(OP(scan)==BACK)? scan - orphanNode: orphanNode - scan;
	//  Begin New code for PocketBrowser.
	if (*sp > 127)
	{
		*(scan+2)=(*sp / 128);
		*(scan+1)=(*sp - (*(scan+2)*128));
	}
	//  end new code for PocketBrowser
	else
	{
 		*(scan+1)=*cp;
		*(scan+2)=*(cp+1);
	}
	delete sp;  //  PocketBrowser modification to free allocated memory
//#endif
}
...
int CRegExp::MatchString(const char *szcStringToMatch)
{
	...
	if (m_efficiencyStartsAnchor)
	{
		...
		if( TestPotentialMatch(szStringToMatch) )
		{
			...
			//  PocketBrowser, modified to properly delete m_szLastMatchedString
			delete[] m_szLastMatchedString;  
			m_szLastMatchedString = NULL;
...
int	CRegExp::MatchStringToPattern(char *compiledExpression)
{
			...
			currentProcessingNode = nextProcessingNode)
	{
		...
		case EXACTLY: 
			...
			//  PocketBrowser - modified to perform case insenstive comparison
			...
			//  PocketBrowser - modified to perform case insensitive comparison.
		...
		case PLUS: 
				...
				swallowedCharacters > min; swallowedCharacters--) 
			{
				...
				//  PocketBrowser - Implemented a case insensitive comparison here.
...
size_t CRegExp::MaxGreedyCharacters(char *operandToGreedy)
{
	...
	//  PocketBrowser - new variables to enable case insensitive comparison for
	//  ANYOF and ANYBUT
	...
	case EXACTLY:
		...
		//  characters.
		//  PocketBrowser - modified to make case insensitive
		count = 0;
	...
	case ANYOF:
		{
		//  PocketBrowser - modification to enable case insensitive comparison.
		...
		//  PocketBrowser - free memory associated with lower case conversion
		delete[] lcaseInputString;
		lcaseInputString = NULL;
		//  End PocketBrowser modifications
		
		return retVal;
		//  PocketBrowser - Original case sensitive return
...
char *CRegExp::NextNode(char *pNode)
{
	...
	//  PocketBrowser - Andreas Phol's change works on mobile devices, 
...
char* CRegExp::ReplaceString( const char* szReplaceExp )
{
	...
	if( szReturnBuffer == NULL )
		return NULL;
	//  PocketBrowser - modified to nullify the memory in the newly 
	//  allocated string
