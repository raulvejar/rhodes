...
void CSIP::InitSIP()
{
	...
	if (hSipWnd) {
		...
		if (m_dwOriginalStyle & WS_CAPTION) {
			/*TCHAR tcText[256];
			wsprintf(tcText, L"%d Left %d Right %d Top %d Bottom", m_rcPosition.left, m_rcPosition.right, m_rcPosition.top, m_rcPosition.bottom);
			MessageBox(SipWnd, tcText, L"",0);*/
			// It has a caption bar so must be CE .NET
			// Now see if its a big one
			m_rcPosition.top += 26;		// Move the SIP down by the CAPTION size
			//m_rcPosition.bottom = m_rcPosition.top+80;	// Adjust the size for the CAPTION removal
			if (m_rcOriginalPos.right - m_rcOriginalPos.left > 242) m_rcPosition.bottom = m_rcPosition.top+194;
			else m_rcPosition.bottom = m_rcPosition.top+80; 
