...
DWORD CEMML::ParseEMMLTag(LPCWSTR szHttpEquiv, LPCWSTR szContents, EMMLPROC lpEMMLProc)
{
	...
	if(m_bDoRegEx)
	{
		...
		{
			regexObj->FindAndReplace(szHttpEquivResult, pbCurPair->szSearchPattern, pbCurPair->szReplacePattern, NULL, &szHttpEquivResult[0]);
			pbCurPair = pbCurPair->lpNextNode;
		}
		pbCurPair = m_RegExPatternsCT;
		while(pbCurPair != NULL)
		{
			regexObj->FindAndReplace(szContentsResult, pbCurPair->szSearchPattern, pbCurPair->szReplacePattern, NULL, &szContentsResult[0]);
			pbCurPair = pbCurPair->lpNextNode;
		}
